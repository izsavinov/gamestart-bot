...
async def helpme(ctx):
    ...
    embed.add_field(name="Добро пожаловать в GameStart!", value='Здесь вы узнаете как пользоваться этим ботом. Для начала, вы должны быть зар'
...
async def on_guild_join(guild):
    ...
    try:
        ...
        await join_chanell.send('На вашем сервере работает Gamestart! Чтобы изучить работу бота, введите команду .helpme')
